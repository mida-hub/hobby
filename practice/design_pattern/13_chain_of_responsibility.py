...
class UserProfileChecker(Handler):
    def __init__(self):
        name_handler = NameCheckHandler()
        age_handler = AgeCheckHandler()
        gender_handler = GenderCheckHandler()

        self.set_next(name_handler) \
            .set_next(age_handler) \
            .set_next(gender_handler)
...
# user1 = User('Taro', 18, 'M')

user_profile_checker = UserProfileChecker()
user_profile_checker.handle(user1)
